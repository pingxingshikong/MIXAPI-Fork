...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		// 增加Token使用次数
		go func() {
			if increaseErr := model.IncreaseTokenUsageCount(token.Key); increaseErr != nil {
				common.SysError("failed to increase token usage count: " + increaseErr.Error())
			}
		}()

		// 记录Token使用日志（用于频率限制）
		go func() {
			if recordErr := model.RecordTokenUsage(token.Id); recordErr != nil {
				common.SysError("failed to record token usage: " + recordErr.Error())
			}
		}()
